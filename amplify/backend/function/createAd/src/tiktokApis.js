...
const findSparkAd = async (token, authCode, advId, pageNum) =>{
    const url=  process.env.TKTOK_BUSNS_API_BASE_URL+`tt_video/list/?advertiser_id=${advId}&page_size=50&page=${pageNum}`;
    console.log(url)
    const headers = { 
        'Access-Token': token,
    }
    ...
    if (data?.code  === 0 && data?.message === "OK"){
        for (let _ad of data?.data?.list){
            if (_ad?.item_info?.auth_code === authCode) {
                    return {item_id: _ad?.item_info?.item_id, identity_id: _ad?.user_info.identity_id
                }
            }
        }
        return data?.data?.page_info
    }
    throw new Error (`Listing Spark Ads returned in error: ${JSON.stringify(data)}`)
}
...
const createAd  = async (token, authCode,advId,adgroupId,userProfileId ) => {
    ...
    if(sprkAd?.code !== 0 || sprkAd?.message !== "OK"){
        throw new Error (`Creating SparkAd returned Error ${JSON.stringify(sprkAd)}`);
    }
    ...
    if(!resp?.item_id){
        throw new Error (`Listing Spark Ads Didnt find any with authCode: ${authCode}`);
    }
    ...
    if(adCreated?.code !== 0 || adCreated?.message !== "OK"){
        throw new Error (`Creating Ad With GroupId  returned Error ${JSON.stringify(adCreated)}`);
    }
    return {
