...
const getS3Object = async (key) =>{
    try {
        await s3.headObject({ Bucket: process.env.CREATIVES_BUCKET, Key: key }).promise()
...
const generateVideoThumbnail = async (token, advId, videoId) => {
    let url=  process.env.TKTOK_BUSNS_API_BASE_URL+`file/video/suggestcover/?advertiser_id=${advId}&video_id=${videoId}`;
    console.log("Prepared url :",url)
    const headers = { 
        'Access-Token': token,
        'Content-Type': "application/json"
...
const getIdentityId = async (token,advId) => {
    const url=  process.env.TKTOK_BUSNS_API_BASE_URL+`identity/get/?advertiser_id=${advId}`;
    console.log("Prepared url :",url)
    const headers = { 
        'Access-Token': token,
        'Content-Type': "application/json"
...
const createAdWithVideoUpload = async (token,adgroupId,advId, videoId, userProfileId) => {
