...
const {
  uploadToTiktok,
  createAdWithVideoUpload,
} = require('./tiktokVideoUpload');

exports.handler = async (event) => {
  console.log(`EVENT: ${JSON.stringify(event)}`);
  try {
    if ((event?.arguments?.landingPageUrl?.split('/')).length > 1) {
      const data = await uploadToTiktok(
        event.arguments.token,
        event.arguments.advId,
        event.arguments.landingPageUrl
      );
      ...
      const adResponse = await createAdWithVideoUpload(
        event.arguments.token,
      ...
      const approvedAdRequest = await createApprovedAD({
        creativeRequestId: event.arguments.creativeRequestId,
        ...adResponse,
      });
