...
const Dashboard: React.FC<ProfileProps> = ({profileState: {data}}) => {
    ...
    if (data?.userType=== USER_TYPES.CREATIVE_USER && isShowModal) return <Modal
        withOutLabel={true}
        ...
        withCheckbox={true}
        checkBoxText={<p>I agree to the <span onClick={() => {
            window.open(`${process.env.REACT_APP_URL}${UnAuthRoutes.TermsAndConditions.slice(1)}`, '_blank')
        }
